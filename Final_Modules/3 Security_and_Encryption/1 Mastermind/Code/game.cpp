...
*/
void GameStart(struct profileSettings *profile) {
	//Gameplayloop
	//Make 4 color code
	FILE *moves;
	...
	char g;
	int i, x, y, total[3], colorCheck, colorFlag, numOfGuesses = 2;
	...
	while (numOfGuesses > 0) {
		...
		if (total[0] == 4) {
			...
			remove("moveset.mm");
			profile->currentRecord.regWin++;
			profile->currentRecord.numberOfRegularGames++;
			return;
		}
	...
	fprintf(stdout, "\n");
	remove("moveset.mm");
	profile->currentRecord.regLoss++;
	profile->currentRecord.numberOfRegularGames++;
	return;
}

/*Function ChallengeGameStart
Description: <Function Description>
-Variables-
aiCode[4]: AI generated peg selection
userPeg[4]: User peg selection
...
Function File Declaration: game.h
*/
void ChallengeGameStart(struct profileSettings *profile) {
	//Gameplayloop
	//Make 4 color code
	FILE *moves;
	enum boardpegs aiCode[4], userPeg[4];
	...
	while (numOfGuesses > 0) {
		...
		if (total[0] == 4) {
			fprintf(stdout, "Congratulations, you successfully cracked the code!\n");
			remove("moveset.mm");
			profile->currentRecord.challWin++;
			profile->currentRecord.numberOfChallengeGames++;
			return;
		}
		//Print out list of prior guesses
		if (numOfGuesses != 0) {
	...
	fprintf(stdout, "\n");
	remove("moveset.mm");
	profile->currentRecord.challLoss++;
	profile->currentRecord.numberOfChallengeGames++;
	return;
}
...
int GetBoardPeg(char guess) {
	...
	PrintError("User Peg Input Error", 2, 0);
	return 0;
}